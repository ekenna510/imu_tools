  ...
  initialized_(false)
{
  ...
  // efk added  
  if (!nh_private_.getParam ("imu_topic",topic_name_))
    topic_name_ = ros::names::resolve("imu");
  if (!nh_private_.getParam ("magnetic_topic",magtopic_name_))
    magtopic_name_ = ros::names::resolve("magnectic_field");
  ...
  // **** register publishers
  imu_publisher_ = nh_.advertise<sensor_msgs::Imu>( topic_name_ + "/data", 5);
